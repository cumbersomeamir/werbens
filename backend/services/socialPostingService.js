...
async function publishToLinkedIn(db, job) {
  ...
  if (!account) {
    throw new Error(`LinkedIn account not found for user ${userId}, channel ${channelId}`);
  }

  if (!account.accessToken) {
    throw new Error(`LinkedIn account missing access token for user ${userId}, channel ${channelId}`);
  }

  // Get user's LinkedIn Person URN (needed for author field)
  // First try to get from userinfo endpoint
  ...
  const postToLinkedIn = async (token) => {
    ...
    return { postId, response: await response.json().catch(() => ({})) };
  };

  // Try posting with current token
  let result;
  try {
    result = await postToLinkedIn(accessToken);
  } catch (err) {
    // If token expired (401), try refreshing
    if (err.message.includes("401") || err.message.includes("Unauthorized") || err.message.includes("expired")) {
      // LinkedIn token refresh would go here if refresh tokens are supported
      // For now, throw error asking user to reconnect
