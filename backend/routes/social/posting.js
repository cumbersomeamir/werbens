...
// Import publishToX directly for immediate posting
async function publishToXDirectly(userId, target, content) {
...
export async function createPostHandler(req, res) {
  ...
  try {
    // For immediate mode, post directly without scheduler
    if (req.body.mode === "immediate") {
      ...
      // For scheduled/automatic mode, use the scheduler
